...
public class SessionUtils {
	...
	public Session getActiveSession(HttpServletRequest request) {
		...
		if(request.getSession().getAttribute(AUTHENTICATED_SESSION)!=null) {
			return (Session) request.getSession().getAttribute(AUTHENTICATED_SESSION);
		}
	...
	public Realm getCurrentRealm(HttpServletRequest request) throws UnauthorizedException {
	...
	public Principal getPrincipal(HttpServletRequest request) throws UnauthorizedException {
		Session session = getActiveSession(request);
		if(session==null)
			throw new UnauthorizedException();
	...
	public Session touchSession(HttpServletRequest request,
			HttpServletResponse response) throws UnauthorizedException, SessionTimeoutException {
	...
	public void addAPISession(HttpServletRequest request, HttpServletResponse response, Session session) {
		...
		cookie.setMaxAge(60 * 15); // TODO this should be configurable
