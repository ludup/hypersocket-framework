...
public class SimpleThreadPoolEx implements ThreadPool {
     ...
     */
    public SimpleThreadPoolEx(int threadCount, int threadPriority) {
    ...
    public void setThreadCount(int count) {
    	if(this.count==-1) {
    		...
    		synchronized(nextRunnableLock) {
	    		int change = count - this.count;
	    		if(change > 0) {
	    			List<WorkerThread> newThreads = createWorkerThreads(change);
	    	        
	    	        for(WorkerThread wt : newThreads) {
	    	            wt.start();
	    	            availWorkers.add(wt);
	    	            workers.add(wt);
	    	        }
	    			...
	    			change = this.count - count;
	    			while(!availWorkers.isEmpty() && change > 0) {
    ...
    public void initialize() throws SchedulerConfigException {
        ...
        if(isThreadsInheritGroupOfInitializingThread()) {
            ...
            threadGroup = new ThreadGroup(parent, schedulerInstanceName + "-SchedulerThreadPool");
    ...
    protected void makeAvailable(WorkerThread wt) {
        synchronized(nextRunnableLock) {
            ...
            if(!isShutdown) {
            	if(pendingThreadShutdown > 0) {
            		wt.shutdown();
            		pendingThreadShutdown--;
            		this.count--;
            	} else {
            		availWorkers.add(wt);		
                    nextRunnableLock.notifyAll();
    ...
    class WorkerThread extends Thread {
         ...
         * </p>
         */
        WorkerThread(SimpleThreadPoolEx tp, ThreadGroup threadGroup, String name,
