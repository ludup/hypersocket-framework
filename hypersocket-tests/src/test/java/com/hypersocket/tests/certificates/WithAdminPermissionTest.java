...
public class WithAdminPermissionTest extends AbstractServerTest {
	...
	public void testKeyGeneration() throws Exception{
		...
		debugJSON(json);
		assertTrue(resp.isSuccess());
		assertEquals(0, resp.getMessage().indexOf("-----BEGIN CERTIFICATE REQUEST-----"));
	...
	public void testUploadKey()throws Exception{
		...
		String json=doPostMultiparts("/hypersocket/api/certificates/key", properties, filePro,fileBundle,Key);
		debugJSON(json);
		JsonResponse resp=getMapper().readValue(json, JsonResponse.class);
		assertTrue(resp.isSuccess());
		assertEquals("Your key has been accepted. The server will need to be restarted.",resp.getMessage());
	...
	public void testUploadPfx() throws Exception{
		...
		String json=doPostMultiparts("/hypersocket/api/certificates/pfx", properties,filePro);
		debugJSON(json);
		JsonResponse resp=getMapper().readValue(json, JsonResponse.class);
		assertTrue(resp.isSuccess());
		assertEquals("Your key has been accepted. The server will need to be restarted.",resp.getMessage());
	...
	public void testUploadCertificate()throws Exception{
		...
		String json=doPostMultiparts("/hypersocket/api/certificates/cert",null,filePro,KeyBundle);
		debugJSON(json);
